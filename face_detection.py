...
def draw_bboxes(img, bboxes, color = (255,0,0)):
	...
	for b in bboxes:
		...
		if prop > 0.5:
			...
			cv2.rectangle(d_img, (int(xmin*w), int(ymin*h)), (int(xmax*w), int(ymax*h)), (0, 255, 0), 4)
	d_img = cv2.cvtColor(d_img, cv2.COLOR_BGR2RGB)
	return d_img
	
def faceD(image, detector):
	''' load image '''
	...
	bboxes = detector.non_max_sup(bboxes, iou_thresh = 0.6)
	...
	# return nms48_img
	cv2.imshow("frame", nms48_img)
	
def main():
	''' load image '''
	image = cv2.imread('known/obama2_0.jpg')
	faceD(image)
